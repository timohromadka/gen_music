...
from .dataset_utils import get_trimmed_waveform, calculate_waveform_length, get_mel_spectrogram

def generate_samples(model, model_args, experiment_args, device, samples_save_dir):
    model.eval()
    
    if model_args.model == 'diffusion':
        if model_args.dataset_type == 'waveform':
        ...
        elif model_args.dataset_type == 'mel-spectrogram':
            print('this')
            waveform_length = calculate_waveform_length(model_args.sample_length, model_args.sample_rate)
            noise = torch.randn(model_args.num_channels, waveform_length).to(device)
            noisy_mel_spectrogram = get_mel_spectrogram(noise, model_args.sample_rate, model_args.n_mels)
            with torch.no_grad():
                for i in tqdm(range(experiment_args.num_batches_to_generate), desc='generating samples'):
                    noise = torch.randn((experiment_args.inference_batch_size,) + noisy_mel_spectrogram.shape).to(device)
                    batch_samples = model.sample(noise, experiment_args.num_steps_for_inference)
